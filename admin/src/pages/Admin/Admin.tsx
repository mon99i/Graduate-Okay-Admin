...
const Admin: React.FC = () => {
  ...
  const fetchAdmins = async (page: number = 0) => {
    try {
      ...
      const sortedAdmins = adminList.sort(
        (a: AdminProps, b: AdminProps) => a.id - b.id,
      );
      ...
      const filteredAdmins =
        loginId === process.env.REACT_APP_ADMIN_ID
          ? sortedAdmins
          : sortedAdmins.filter(
              (admin: AdminProps) => admin.loginId === loginId,
            );
    ...
    <div className="container mx-96 py-16">
      ...
      <table className="min-w-full bg-white border border-gray-200">
        <thead>
          <tr className="w-full bg-gray-100 border">
            ...
            <th className="border-x py-2 px-4 text-left text-gray-600">
              관리자 아이디
            </th>
            <th className="border-x py-2 px-4 text-left text-gray-600">
              생성일
            ...
            <th className="border-x py-2 px-2 text-center text-sm text-gray-600">
              수정
            </th>
            <th className="border-x py-2 px-2 text-center text-sm text-gray-600">
              삭제
      ...
      <div className="mt-8">
        <ReactPaginate
          ...
          pageClassName={'mx-1'}
          pageLinkClassName={
            'px-4 py-2 border border-gray-300 rounded-full hover:bg-gray-200 flex items-center justify-center'
          }
